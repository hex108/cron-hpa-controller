...
type CronHPALister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.CronHPA, err error)
...
// List lists all CronHPAs in the indexer.
func (s *cronHPALister) List(selector labels.Selector) (ret []*v1alpha1.CronHPA, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CronHPA))
	})
	return ret, err
}
...
type CronHPANamespaceLister interface {
	...
	Get(name string) (*v1alpha1.CronHPA, error)
...
// List lists all CronHPAs in the indexer for a given namespace.
func (s cronHPANamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.CronHPA, err error) {
...
// Get retrieves the CronHPA from the indexer for a given namespace and name.
func (s cronHPANamespaceLister) Get(name string) (*v1alpha1.CronHPA, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("cronhpa"), name)
	}
	return obj.(*v1alpha1.CronHPA), nil
}
